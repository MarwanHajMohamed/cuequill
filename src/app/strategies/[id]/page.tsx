...
function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="w-full max-w-[1500px] mx-auto px-4 md:px-8 pt-24 pb-6 flex flex-col gap-4">
      ...
      {/* Videos — a per-strategy folder. Upload/delete persist immediately,
          independent of the edit form's Save. */}
      <StrategyVideos strategyId={data._id} videos={data.videos ?? []} />
...
// ── Videos folder ─────────────────────────────────────────────────────
// A per-strategy video folder. Files upload straight to Vercel Blob from
// the browser (via a scoped upload token), then the returned URL is saved
// against the strategy. Managed independently of the edit form, so
// upload/delete take effect immediately.
  ...
  const onPick = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const blob = await upload(file.name, file, {
        access: "public",
        handleUploadUrl: `/api/strategies/${strategyId}/video-upload`,
        contentType: file.type || undefined,
        onUploadProgress: ({ percentage }) =>
          setProgress(Math.round(percentage)),
      });
      const res = await fetch(`/api/strategies/${strategyId}/videos`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          url: blob.url,
      ...
      if (!res.ok) {
        ...
        throw new Error(d.error ?? "Couldn't save the video.");
      }
      await refresh();
    } catch (e2) {
  ...
  const remove = async (videoId: string) => {
    ...
    try {
      const res = await fetch(
        `/api/strategies/${strategyId}/videos/${videoId}`,
        { method: "DELETE" },
      );
      if (!res.ok) throw new Error("Couldn't delete the video.");
      await refresh();
    } catch (e2) {
    ...
    <div className="flex flex-col gap-2.5">
      <div className="flex items-center justify-between gap-2 px-1">
        ...
        <button
          ...
          disabled={uploading}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11.5px] font-medium border border-white/12 bg-white/[0.03] text-white/80 hover:bg-white/[0.06] hover:text-white transition cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
        >
          <i
            className={`fa-solid ${
              uploading ? "fa-spinner fa-spin" : "fa-arrow-up-from-bracket"
            } text-[11px]`}
          />
      ...
      {videos.length === 0 ? (
        !uploading && (
          <button
            type="button"
            onClick={() => fileRef.current?.click()}
            className="rounded-2xl border border-dashed border-white/12 bg-white/[0.02] hover:bg-white/[0.04] hover:border-white/25 transition p-8 text-center cursor-pointer"
          >
            <i className="fa-solid fa-film text-white/30 text-lg" />
            <div className="mt-2 text-[13px] text-white/55">
              Upload videos for this strategy
            </div>
            <div className="mt-0.5 text-[11px] text-white/35">
              Recordings, walkthroughs, or examples. MP4, MOV, WebM.
            </div>
          </button>
        )
      ) : (
            ...
            >
              ...
              <div className="flex items-center gap-2 px-3 py-2">
                ...
                <button
                  ...
                  className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-white/35 hover:text-red-300 hover:bg-red-500/10 transition cursor-pointer disabled:opacity-50"
                >
                  <i
                    className={`fa-solid ${
                      deletingId === v.id
